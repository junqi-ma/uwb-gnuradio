...
**这给出"商用设备抵消不理想"的判读（注意到 2026-09-14 实机验证后已修正）**：实机扫描
（见 `测试报告_X410双TX_jammer_CFO零点.md`）显示**坏区（梳齿峰）只有 ±~7 kHz 宽、
占空比 ~1.4%**，中间整段都被压制到本底，零点精确落在峰旁 8.62 kHz。因此随机/漂移的
CFO **大概率**落在被压制区 —— 商用场景下残余干扰更可能来自**周期性被破坏**（商用设备
符号时钟 ≠ X410，114 次平均里采样相位滑移）、脉冲形状失配、非 preamble 段与 RX 非线性，
而不是"CFO 恰好踩在梳齿峰上"，也难归因于单纯 TX EVM。这正是双 TX 实验已经分离出来的。
