...
class Step12SelfTest(unittest.TestCase):
    ...
    def test_dry_run_default_plan(self):
        ...
        self.assertEqual(plan["psdu"]["mode"], "empty")
        self.assertEqual(plan["psdu"]["psdu_length_bytes"], 0)
        self.assertTrue(plan["psdu"]["default_empty"])
    ...
    def test_cg400_rate_dry_run(self):
        r = run_cli(["--dry-run", "--rate", "491.52e6"])
        self.assertEqual(r.returncode, 0, r.stdout + r.stderr)
        plan = json.loads(r.stdout)
        self.assertEqual(plan["rate"]["hz"], 491520000.0)
        self.assertEqual(plan["rate"]["resample"], "65/32")
        self.assertEqual(plan["pri"]["ticks"], 2457600)
        w = plan["windows"]
        self.assertEqual(w["pre_guard_native"], 983)
        self.assertEqual(w["sync_native"], 32012)
        self.assertEqual(w["sfd_native_4z2"], 4002)
        self.assertEqual(w["range_guard_native"], 50)
        self.assertEqual(w["resampler_tail_native"], 2015)
        self.assertEqual(w["rx_window_native"], 39062)
        self.assertEqual(w["resample_decim"], 32)

    def test_sync_reps_and_psdu_choices(self):
        r = run_cli(["--dry-run", "--sync-reps", "48"])
        self.assertEqual(r.returncode, 1)
        self.assertIn("cli_invalid_arg", r.stdout)
        r = run_cli(["--dry-run", "--sync-reps", "2048"])
        self.assertEqual(r.returncode, 0, r.stdout + r.stderr)
        plan = json.loads(r.stdout)
        self.assertEqual(plan["windows"]["sync_reps"], 2048)
        self.assertGreater(plan["windows"]["rx_window_native"], 1 << 20)
        self.assertLessEqual(plan["windows"]["rx_window_native"], 1 << 21)
        r = run_cli(["--dry-run", "--psdu-hex", "AABBCC"])
        self.assertEqual(r.returncode, 0, r.stdout)
        plan = json.loads(r.stdout)
        self.assertEqual(plan["psdu"]["mode"], "hex")
        self.assertEqual(plan["psdu"]["psdu_length_bytes"], 3)
        self.assertEqual(plan["psdu"]["hex"], "AABBCC")
        r = run_cli(["--dry-run", "--psdu-bytes", "20"])
        self.assertEqual(r.returncode, 0, r.stdout)
        plan = json.loads(r.stdout)
        self.assertEqual(plan["psdu"]["mode"], "random_data_plus_fcs")
        self.assertEqual(plan["psdu"]["psdu_length_bytes"], 22)
        r = run_cli(["--dry-run", "--psdu-bytes", "126"])
        self.assertEqual(r.returncode, 1)
        self.assertIn("psdu_too_long", r.stdout)
