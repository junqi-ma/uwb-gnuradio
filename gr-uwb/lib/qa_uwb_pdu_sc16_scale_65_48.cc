 ...
 *
 * QA for the SC16 amplitude contract of UwbPduRationalResamplerCcf65_48
 * (737.28 -> 998.4 MS/s, fixed 65/48).  Contract: round-2 spec section A
 ...
 * (resampler_65_48/taps_quality_minorder.txt, as the existing QA does),
...
using Sc16ScalePolicy = UwbPduRationalResamplerCcf65_48::Sc16ScalePolicy;
...
bool try_load_real_taps(std::vector<float>& out)
{
    ...
    const std::string path =
        ...
        "/resampler_65_48/taps_quality_minorder.txt";
...
RunResult run_pdu(UwbPduRationalResamplerCcf65_48::sptr blk,
                  pmt::pmt_t pdu,
                  int timeout_ms = 10000)
{
    auto dbg = gr::blocks::message_debug::make();
    auto st = gr::blocks::message_debug::make();
    auto tb = gr::make_top_block("qa_pdu_sc16_scale_65_48");
...
// ---------------------------------------------------------------------------
BOOST_AUTO_TEST_CASE(test_65_48_sc16_unit_range_matches_fc32)
{
    const auto taps = test_taps();

    auto blk_a = UwbPduRationalResamplerCcf65_48::make_from_taps(
        taps, kOutRate, true,
        UwbPduRationalResamplerCcf65_48::EmitPolicy::FullWindow, kMaxIn,
        Sc16ScalePolicy::UnitRange);
    auto blk_b = UwbPduRationalResamplerCcf65_48::make_from_taps(
        taps, kOutRate, true,
        UwbPduRationalResamplerCcf65_48::EmitPolicy::FullWindow, kMaxIn); // default RawInteger
    auto blk_c = UwbPduRationalResamplerCcf65_48::make_from_taps(
        taps, kOutRate, true,
        UwbPduRationalResamplerCcf65_48::EmitPolicy::FullWindow, kMaxIn,
...
// ---------------------------------------------------------------------------
BOOST_AUTO_TEST_CASE(test_65_48_published_samples_absent_stays_absent)
{
