#!/usr/bin/env python3
"""Generate the production 491.52 MS/s QM35 code-9 SYNC template.

Contract (docs/phase1/GROK_X410_QM35盲捕获与自动周期锁定开发方案.md §8,
adapted from the 737.28 MS/s variant):

  testdata/reference_preamble.bin          # 998.4 MS/s, one SYNC, L2-normalized
        -- resample_poly 32/65 -->
  testdata/reference_preamble_code9_491p52.cf32
  testdata/reference_preamble_code9_491p52_metadata.json
...
65/48 host resampler and the 737.28 MS/s 48/65 generator of record
(testdata/generate_qm35_reference_737p28.py). Group delay is recorded so
C++ / MATLAB comparisons can apply a documented
1-sample-or-filter-delay tolerance.

This script is the generator of record for the 491.52 MS/s native rate
(X410 2x decimation path: 998.4 * 32 / 65 = 491.52 MS/s) when MATLAB is
not available. It mirrors generate_qm35_reference_737p28.py with
INTERP=32, DECIM=65.
"""
...
DEFAULT_OUT = os.path.join(HERE, "reference_preamble_code9_491p52.cf32")
DEFAULT_META = os.path.join(HERE, "reference_preamble_code9_491p52_metadata.json")
...
def generate(src_path: str, out_path: str, meta_path: str) -> dict:
    ...
    meta = {
        "description": (
            ...
            "buildUwbReference.m template by polyphase 32/65 resample."
        ),
        "code_index": 9,
        ...
        "generator": "testdata/generate_qm35_reference_491p52.py",
